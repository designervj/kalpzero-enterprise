...
import { ArrowLeft, Save, Terminal, Type, Hash, Info, MapPin, Globe, CheckCircle2 } from "lucide-react";
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black text-white uppercase tracking-tighter">
              {initialData ? "Update" : "Add"} <span className="text-gold">Warehouse</span>
            </h1>
          </div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-white/40 uppercase tracking-widest italic">
            ...
            {initialData ? `Modifying Warehouse: ${initialData.name}` : "Onboarding New Warehouse"}
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="bg-charcoal border border-white/5 rounded-sm p-6 space-y-6 shadow-2xl shadow-black/40 relative overflow-hidden group">
            ...
            <div className="flex items-center gap-3 border-l-2 border-gold pl-4">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-[0.2em]">Warehouse details</h3>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-2">
                  <Type size={12} className="text-gold" /> Warehouse Name
                </label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-white/40 uppercase tracking-widest flex items-center gap-2">
                  <Hash size={12} className="text-gold" /> Warehouse Code
                </label>
                <input
                  type="text"
                  value={formData.code}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-12 bg-ink border border-white/10 rounded-sm pl-28 pr-4 text-xs font-bold text-gold lowercase tracking-widest focus:border-gold outline-none transition-all"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-charcoal border border-white/5 rounded-sm p-6 space-y-6 shadow-2xl shadow-black/40 relative overflow-hidden group">
            <div className="flex items-center gap-3 border-l-2 border-gold pl-4">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-[0.2em]">Geo Location</h3>
            </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-charcoal border border-white/5 rounded-sm p-6 space-y-6 shadow-2xl shadow-black/40">
            <div className="flex items-center gap-4 border-l-2 border-gold pl-4">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-[0.2em]">Configuration</h3>
            </div>
            ...
            <button 
              ...
              className={`w-full p-4 border rounded-sm flex items-center justify-between transition-all group ${
                formData.is_default ? "bg-white/5 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]" : "bg-ink border-white/5 opacity-40 hover:opacity-100"
              }`}
            >
              <div className="flex flex-col items-start gap-1">
                <span className={`text-[10px] font-black uppercase tracking-widest ${formData.is_default ? "text-emerald-500" : "text-white/40"}`}>Default Warehouse</span>
                <span className="text-[8px] font-bold text-white/20 uppercase">Set as primary storage node</span>
              </div>
              <div className={`h-5 w-5 border rounded-sm flex items-center justify-center transition-all ${formData.is_default ? "bg-emerald-500 border-emerald-500" : "border-white/10"}`}>
                {formData.is_default && <CheckCircle2 size={12} className="text-ink" />}
              </div>
            </button>
          </div>
        </div>
