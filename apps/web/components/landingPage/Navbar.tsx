...
export default function Navbar({ setView, currentView }: NavbarProps) {
  ...
  return (
    <motion.nav 
      ...
      className={`fixed top-0 left-0 right-0 z-50 px-6 py-3 backdrop-blur-md border-b transition-colors duration-300 ${
        themeMode === 'light' 
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        >
          <img
            src={themeMode === 'light' ? "/img/favicon-img.svg" : "/img/white-logo-kalptree.svg"}
            alt="KalpTree Logo"
        ...
        {/* Desktop Nav */}
        <div className={`hidden lg:flex items-center gap-6 text-sm font-medium transition-colors ${
          ...
          <Link
            href="/docs"
            className={`relative px-1 py-2 transition-colors hover:text-pink-400 ${
              themeMode === 'light' ? 'text-slate-600' : 'text-gray-300'
            }`}
          >
            Docs
        ...
        <div className="hidden lg:flex items-center gap-3">
            ...
            <Link
              ...
              className={`inline-flex items-center justify-center rounded-full border px-5 py-2.5 text-sm font-medium transition-colors ${
                themeMode === 'light' 
                  ...
                  : 'border-white/15 text-white hover:border-fuchsia-400/50 hover:bg-white/5'
              }`}
            >
              Login
            </Link>
          ...
          <motion.button 
            ...
            className={`flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white rounded-full transition-all shadow-lg ${
              themeMode === 'light'
      ...
      {isOpen && (
        <motion.div 
          ...
          className={`lg:hidden pt-4 pb-6 px-4 flex flex-col gap-4 border-t mt-4 rounded-2xl transition-colors duration-500 ${
            themeMode === 'light' 
            ...
            <button 
              ...
              className={`text-left py-2 transition-colors ${
                currentView === item.id 
                  ...
                  : themeMode === 'light' ? 'text-slate-600 hover:text-pink-400' : 'text-gray-300 hover:text-pink-400'
              }`}
            >
              {item.label}
            </button>
