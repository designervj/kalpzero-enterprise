...
import { Upload, X, FileJson, CheckCircle2, AlertCircle, Download, FolderTree } from 'lucide-react';
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="w-full max-w-4xl bg-[#0a0a0c] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-gradient-to-r from-purple-500/10 to-transparent">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-white leading-tight">Import Categories</h2>
                            <p className="text-sm text-slate-400">Upload a JSON file to bulk create categories.</p>
                        </div>
                    </div>
                    ...
                    {!results && (
                        <div 
                            className={`border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center gap-4 cursor-pointer
                                ${file ? 'border-purple-500/50 bg-purple-500/5' : 'border-slate-800 hover:border-slate-700 bg-slate-900/40 hover:bg-slate-900/60'}`}
                            onClick={() => fileInputRef.current?.click()}
                        >
                            <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".json" className="hidden" />
                            <div className={`w-16 h-16 rounded-full flex items-center justify-center ${file ? 'bg-purple-500 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                <Upload size={32} />
                            </div>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); downloadSample(); }} className="mt-2 text-xs text-purple-400 hover:text-purple-300 flex items-center gap-1.5 font-medium transition-colors">
                        ...
                        <div className="space-y-6 animate-in zoom-in-95 duration-300">
                            <div className="p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-emerald-300/80">{results.count} categories created successfully.</p>
                                </div>
                            </div>
                            ...
                            <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-900/20">
                                <table className="w-full text-left text-sm border-collapse">
                                    <thead>
                                        <tr className="bg-slate-900/60 border-b border-slate-800">
                                            ...
                                            <th className="px-4 py-3 font-bold text-slate-300">Slug</th>
                                            <th className="px-4 py-3 font-bold text-slate-300">Type</th>
                                            <th className="px-4 py-3 font-bold text-slate-300">Parent</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-800">
                                        {previewData.slice(0, 10).map((cat, i) => (
                                            <tr key={i} className="hover:bg-slate-800/30 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-slate-500">{cat.parentId || 'None'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                {previewData.length > 10 && (
                                    <div className="p-3 text-center border-t border-slate-800 bg-slate-900/40">
                                        <p className="text-xs text-slate-500 font-medium">... and {previewData.length - 10} more categories</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <div className="p-6 border-t border-slate-800 bg-slate-900/40 flex items-center justify-end gap-3">
                    ...
                    {!results && (
                        <button 
                            ...
                            className="px-8 py-2.5 rounded-xl bg-purple-500 hover:bg-purple-400 text-black font-bold text-sm shadow-lg shadow-purple-500/20 disabled:opacity-50 disabled:shadow-none transition-all flex items-center gap-2"
                        >
                            {importing ? (
                                <>
