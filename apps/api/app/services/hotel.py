...
def _serialize_reservation(model) -> dict[str, object]:
    return {
        ...
        "guest_customer_id": str(model.guest_customer_id),
...
def _serialize_room_move(model) -> dict[str, object]:
    return {
        ...
        "reason": model.reason,
        "moved_by_user_id": str(model.moved_by_user_id),
        "created_at": model.created_at.isoformat(),
    }
...
def _serialize_night_audit(model) -> dict[str, object]:
    return {
        ...
        "completed_by_user_id": str(model.completed_by_user_id),
        "created_at": model.created_at.isoformat(),
    }
...
def _room_state_from_request(*, status: str, occupancy_status: str | None, housekeeping_status: str | None, sell_status: str | None) -> dict[str, str]:
    presets = {
...
def _set_room_state(room, *, occupancy_status: str | None = None, housekeeping_status: str | None = None, sell_status: str | None = None, last_cleaned_at: str | None = None) -> None:
...
def _booking_reference(reservation) -> str:
    return reservation.booking_reference or f"HK-{str(reservation.id)[:8].upper()}"


def _invoice_number(folio) -> str:
    return folio.invoice_number or f"INV-{str(folio.id)[:8].upper()}"


async def _recalculate_folio_totals(db: Session, folio, db_name: str) -> None:
    ...
    refunds = await hotel_repository.list_refunds(db_name, folio_id=folio.id)
...
async def get_overview(db: Session, *, tenant_slug: str, db_name: str) -> dict[str, object]:
    ...
    meal_plans = await hotel_repository.list_meal_plans(db_name)
    guest_profiles = await hotel_repository.list_guest_profiles(db_name)
    guest_documents = []
    for guest_profile in guest_profiles:
        guest_documents.extend(
            await hotel_repository.list_guest_documents(db_name, guest_profile_id=guest_profile.id)
        )
    rate_plans = await hotel_repository.list_rate_plans(db_name)
    availability_rules = await hotel_repository.list_availability_rules(db_name)
    reservations = await hotel_repository.list_reservations(db_name)
    stays = await hotel_repository.list_stays(db_name)
    folios = await hotel_repository.list_folios(db_name)
    payments = await hotel_repository.list_payments(db_name)
    refunds = await hotel_repository.list_refunds(db_name)
    staff_members = await hotel_repository.list_staff_members(db_name)
    shifts = await hotel_repository.list_shifts(db_name)
    night_audits = await hotel_repository.list_night_audits(db_name)
    housekeeping_tasks = await hotel_repository.list_housekeeping_tasks(db_name)
    maintenance_tickets = await hotel_repository.list_maintenance_tickets(db_name)
...
async def create_property(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, name: str, code: str, city: str, country: str, timezone: str) -> dict[str, object]:
    ...
    model = await hotel_repository.create_property(
        db_name,
        name=name,
        code=code,
        city=city,
...
async def create_room_type(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, name: str, code: str, category: str | None, bed_type: str | None, occupancy: int, room_size_sqm: int | None, base_rate_minor: int, extra_bed_price_minor: int, refundable: bool, currency: str, amenity_ids: list[str]) -> dict[str, object]:
    ...
    model = await hotel_repository.create_room_type(
        db_name,
...
async def create_room(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_type_id: str, room_number: str, status: str, occupancy_status: str | None, housekeeping_status: str | None, sell_status: str | None, is_active: bool, feature_tags: list[str], notes: str | None, last_cleaned_at: str | None, floor_label: str | None) -> dict[str, object]:
    ...
    if await hotel_repository.find_room_by_number(db_name, property_id=property_id, room_number=room_number):
    ...
    model = await hotel_repository.create_room(
        db_name,
        property_id=property_id,
...
async def create_meal_plan(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, code: str, name: str, description: str | None, price_per_person_per_night_minor: int, currency: str, included_meals: list[str], is_active: bool) -> dict[str, object]:
    ...
    meal_plan = await hotel_repository.create_meal_plan(
        db_name,
        property_id=property_id,
        code=code,
        name=name,
...
async def create_guest_profile(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, first_name: str, last_name: str, email: str, phone: str, nationality: str | None, loyalty_tier: str | None, vip: bool, preferred_room_type_id: str | None, dietary_preference: str | None, company_name: str | None, identity_document_number: str | None, notes: str | None) -> dict[str, object]:
    ...
    if preferred_room_type_id:
        await _room_type_or_raise(db_name, tenant_id=tenant_slug, room_type_id=preferred_room_type_id)

    guest_profile = await hotel_repository.create_guest_profile(
        db_name,
...
async def list_guest_documents(db: Session, *, tenant_slug: str, db_name: str, guest_profile_id: str) -> list[dict[str, object]]:
    ...
    return [
        ...
        for item in await hotel_repository.list_guest_documents(
            db_name,
...
async def create_rate_plan(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_type_id: str, label: str, currency: str, weekend_enabled: bool, weekend_rate_minor: int | None, seasonal_overrides: list[dict[str, object]], is_active: bool) -> dict[str, object]:
    ...
    if room_type.property_id != property_id:
        raise ConflictError("Room type does not belong to the provided property.")
    if await hotel_repository.find_rate_plan_by_label(db_name, room_type_id=room_type_id, label=label):
    ...
    rate_plan = await hotel_repository.create_rate_plan(
        db_name,
        property_id=property_id,
        room_type_id=room_type_id,
        label=label,
...
async def create_availability_rule(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_type_id: str, total_units: int, available_units_snapshot: int | None, minimum_stay_nights: int, maximum_stay_nights: int, blackout_dates: list[str], is_active: bool) -> dict[str, object]:
    ...
    if await hotel_repository.find_availability_rule_by_room_type(db_name, room_type_id=room_type_id):
        raise ConflictError("Availability rule already exists for this room type.")

    availability_rule = await hotel_repository.create_availability_rule(
        db_name,
        property_id=property_id,
        room_type_id=room_type_id,
        total_units=total_units,
...
async def create_reservation(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_type_id: str, room_id: str | None, meal_plan_id: str | None, booking_reference: str | None, booking_source: str | None, guest_customer_id: str, guest_name: str | None, check_in_date: date, check_out_date: date, status: str, special_requests: str | None, early_check_in: bool, late_check_out: bool, total_amount_minor: int, currency: str, adults: int, children: int) -> dict[str, object]:
    ...
    if room_id is not None:
        ...
        if await hotel_repository.find_conflicting_reservation(
            db_name,
            room_id=room_id,
    ...
    if meal_plan_id is not None:
        meal_plan = await _meal_plan_or_raise(db_name, tenant_id=tenant_slug, meal_plan_id=meal_plan_id)
        if meal_plan.property_id != property_id:
            raise ConflictError("Meal plan does not belong to the provided property.")

    guest_profile = await hotel_repository.get_guest_profile(db_name, guest_profile_id=guest_customer_id)
    ...
    reservation = await hotel_repository.create_reservation(
        db_name,
        property_id=property_id,
        room_type_id=room_type_id,
        room_id=room_id,
    ...
    if total_amount_minor > 0:
        await hotel_repository.create_folio_charge(
            db_name,
            folio_id=str(folio.id),
            reservation_id=str(reservation.id),
...
async def record_room_move(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, stay_id: str, to_room_id: str, reason: str) -> dict[str, object]:
    ...
    reservation = await _reservation_or_raise(db_name, tenant_id=tenant_slug, reservation_id=stay.reservation_id)
    from_room = await _room_or_raise(db_name, tenant_id=tenant_slug, room_id=stay.room_id)
    to_room = await _room_or_raise(db_name, tenant_id=tenant_slug, room_id=to_room_id)
    ...
    await hotel_repository.create_room_move(
        db_name,
...
async def add_folio_charge(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, folio_id: str, category: str, label: str, service_date: date, quantity: int, unit_amount_minor: int, tax_amount_minor: int, notes: str | None) -> dict[str, object]:
    ...
    charge = await hotel_repository.create_folio_charge(
        db_name,
        folio_id=str(folio.id),
        reservation_id=str(folio.reservation_id),
        category=category,
...
async def record_payment(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, folio_id: str, amount_minor: int, payment_method: str, reference: str | None, notes: str | None) -> dict[str, object]:
    ...
    payment = await hotel_repository.create_payment(
        db_name,
...
async def record_refund(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, folio_id: str, payment_id: str, amount_minor: int, reason: str, reference: str | None) -> dict[str, object]:
    ...
    existing_refunds = await hotel_repository.list_refunds(db_name, folio_id=folio.id)
    ...
    refund = await hotel_repository.create_refund(
        db_name,
        property_id=folio.property_id,
        folio_id=str(folio.id),
...
async def create_staff_member(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, staff_code: str, first_name: str, last_name: str, role: str, department: str, phone: str | None, email: str | None, employment_status: str, is_active: bool) -> dict[str, object]:
    ...
    staff_member = await hotel_repository.create_staff_member(
        db_name,
        property_id=property_id,
        staff_code=staff_code,
        first_name=first_name,
...
async def create_shift(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, staff_member_id: str, shift_date: date, shift_kind: str, start_time: str, end_time: str, status: str, notes: str | None) -> dict[str, object]:
    ...
    shift = await hotel_repository.create_shift(
        db_name,
        property_id=property_id,
        staff_member_id=staff_member_id,
        shift_date=shift_date,
...
async def run_night_audit(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, audit_date: date) -> dict[str, object]:
    ...
    reservations = await hotel_repository.list_reservations(db_name, property_id=property_id)
    folios = await hotel_repository.list_folios(db_name, property_id=property_id)
    payments = await hotel_repository.list_payments(db_name, property_id=property_id)
    housekeeping_tasks = await hotel_repository.list_housekeeping_tasks(db_name, property_id=property_id)
    maintenance_tickets = await hotel_repository.list_maintenance_tickets(db_name, property_id=property_id)
    rooms = await hotel_repository.list_rooms(db_name, property_id=property_id)
...
async def create_housekeeping_task(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_id: str, priority: str, notes: str | None, assigned_staff_id: str | None, assigned_to: str | None) -> dict[str, object]:
    ...
    if assigned_staff_id is not None:
        staff_member = await _staff_member_or_raise(db_name, tenant_id=tenant_slug, staff_member_id=assigned_staff_id)
        if staff_member.property_id != property_id:
    ...
    task = await hotel_repository.create_housekeeping_task(
        db_name,
        property_id=property_id,
        room_id=room_id,
        status="pending",
...
async def create_maintenance_ticket(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, property_id: str, room_id: str | None, title: str, description: str | None, priority: str, assigned_staff_id: str | None, assigned_to: str | None) -> dict[str, object]:
    ...
    ticket = await hotel_repository.create_maintenance_ticket(
        db_name,
        property_id=property_id,
        room_id=room_id,
        title=title,
...
async def update_maintenance_status(db: Session, *, tenant_slug: str, db_name: str, actor_user_id: str, ticket_id: str, status: str) -> dict[str, object]:
    ...
    if ticket.room_id and status == "resolved":
        room = await _room_or_raise(db_name, tenant_id=tenant_slug, room_id=ticket.room_id)
        other_open_tickets = [
            item for item in await hotel_repository.find_open_maintenance_tickets_for_room(db_name, room_id=room.id)
...
async def get_report_summary(db: Session, *, tenant_slug: str, db_name: str, property_id: str | None, from_date: date, to_date: date) -> dict[str, object]:
    ...
    folios = await hotel_repository.list_folios(db_name, property_id=property_id)
    payments = await hotel_repository.list_payments(db_name, property_id=property_id)
    refunds = await hotel_repository.list_refunds(db_name, property_id=property_id)
    housekeeping_tasks = await hotel_repository.list_housekeeping_tasks(db_name, property_id=property_id)
    maintenance_tickets = await hotel_repository.list_maintenance_tickets(db_name, property_id=property_id)
    night_audits = await hotel_repository.list_night_audits(db_name, property_id=property_id)
    ...
    for stay in filtered_stays:
        room_moves.extend(await hotel_repository.list_room_moves(db_name, stay_id=stay.id))
